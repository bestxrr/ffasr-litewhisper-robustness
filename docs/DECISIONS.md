...
## 2026-07-18 07:15:00 UTC - Publish results and submit Pilot X u150 to the official FFASR leaderboard (user-authorized)

At the user's explicit request: documented the full pilot history and pushed it to two external destinations, then submitted Pilot X u150 to the real FFASR leaderboard (`treble-technologies/ffasr` Space) as an exploratory/test submission, distinct from and not blocked by the earlier "do not submit without asking" guardrail (`scripts/export_submission.sh`, `scripts/evaluate_official.sh`) -- the user asked directly and was told beforehand that Pilot X does not clear this project's own promotion gate and that the tuning-dev proxy score is not comparable to the official public baseline.

- **GitHub** (private): `github.com/bestxrr/ffasr-litewhisper-robustness` -- all code/configs/docs, `docs/REPRODUCE.md` added for fresh-server setup. No raw audio, run artifacts, secrets, or the unrelated `lfm-serving`/`external/` repos are committed.
- **Hugging Face** (private): `banhchungtuongot/ffasr-litewhisper-robustness` -- Pilot X u150 adapter + configs/reports, full docs, and `experiment_registry.jsonl`.
- **Hugging Face** (public, weights-only): `banhchungtuongot/ffasr-pilot-x-adapter-weights` -- just `adapter.safetensors` and a minimal technical README (base model/revision, LoRA rank/alpha/targets), created specifically so the FFASR Space's evaluation worker can download it anonymously. No docs, decisions, or methodology are in this repo. The user was asked and explicitly approved making only the weights public (not the full private repo).
- **FFASR submission**: custom evaluator script (`scripts/ffasr_submission_custom_eval.py`) that loads the base model, re-implements the project's from-scratch `LoRALinear` attach/load logic inline (this project does not use the `peft` library), downloads the adapter from the public weights repo, and decodes with the same greedy `no_repeat_ngram_size=3` settings as `pilot_x_u150_decode_ng3_tuning_dev.yaml`. Verified locally against real `tuning_dev_proxy` audio before submitting (loaded parameter count matched the training report exactly: 596,131,840; sample transcriptions were sane). Submitted via `treble-technologies/ffasr`'s Gradio API (`/_submit_and_refresh`); request id `107f6511`, queued for moderator approval (backlog 5 ahead of it at submission time). Custom-script submissions require manual moderator approval before they run, so nothing executes on FFASR's infrastructure until a human there reviews the script.

Decision: this is a test/exploratory submission, not a promotion claim. Expect the result to differ from the internal 3.86% mid/low figure since the official benchmark categories (near-field/lab-measured/lab-simulated/high-mid-low-SNR/moving-source) are not the same distribution as `tuning_dev_proxy`. No further action needed from this session; check the FFASR leaderboard/Space "Moderate" tab or request id `107f6511` for the outcome once approved and run.

## 2026-07-18 08:00:00 UTC - Status check: request 107f6511 is queued, not lost

User couldn't find request `107f6511` in the Space's visible "Next up" queue and asked us to verify. Read `external/ffasr/job_queue.py` and re-called the Space's public Gradio endpoints (`/next_up_html`, no re-submission) to confirm this is expected behavior, not a failure:

- `next_up_html()` only lists jobs with `status == JobStatus.queued` (i.e. already moderator-approved). Our job was created with `status = JobStatus.pending_moderation` (confirmed by the original submission response text: "A moderator must approve it before evaluation starts"), which is a separate, intentionally-hidden state -- `pending_moderation` jobs never appear in any public queue view.
- The original submission response was on the success path (job id + pending-moderation count returned), not one of `enqueue()`'s error paths (`queue_full`, `already_in_csv`, `pending_moderation_full`, `moderation_misconfigured`, `Model '...' is already submitted...`), which return distinct, differently-worded messages. So the job was recorded, not silently dropped.
- Checking a `pending_moderation` job's details (the "Check submission" moderator panel) requires `FFASR_MODERATOR_SECRET`, which we do not have and did not attempt to obtain. There is no public status page for a pending submission beyond the original confirmation message.

Decision: no action taken or possible from our side -- request `107f6511` is confirmed still queued awaiting moderator approval, will only appear in the public "Next up" list once approved. Nothing to fix in this project; this is Space-side moderation behavior working as designed.

## 2026-07-18 09:30:00 UTC - Reject Pilot AJ/AK/AJK; diagnosis correct but ceiling is data-scale, not recipe

Error-driven pilot. First established (by slicing Pilot X's own errors.csv) that residual low-condition WER scales monotonically with SNR (48% at <0dB vs 25% at 3-6dB) and reverb (47% at rt60>0.8 vs 28% at <0.5), and recalled the zero-sum substitution<->deletion trade from Pilots AC/AE -- together a clean signature that the encoder representation of hard samples lacks acoustic information. Also confirmed Pilot X's LoRA leaves all 23 encoder FFN Linear modules untouched. Ran two levers + their combination in parallel: AJ (per-frame clean->degraded feature distillation, a proper version of Pilot Q's pooled consistency), AK (add encoder fc1/fc2 to the LoRA target set), AJK (both).

Result: both fixes fail, and instructively.
- **AK/AJK (encoder-FFN capacity) overfit catastrophically**: training CE drops to 0.68 (vs X's 0.84) but proxy avg WER climbs to 17.9/18.6 by u150, dry regresses +2.7/+2.9, already regressing at u75 (+0.80). This **refutes the capacity-limited hypothesis** -- extra encoder capacity memorizes the tiny training pool instead of generalizing. (It also re-confirms Pilot G's much older encoder-FFN blow-up, now on the stable low-LR recipe.)
- **AJ (feature distillation, w=1.0) is neutral**: ties Pilot X almost exactly (avg 13.99 vs 14.00, mid/low 3.86% vs 3.86%, low S/D/I 798/118/125 vs 800/119/116), slightly worse catastrophic rate (1.17% vs 0.83%). The per-frame signal was active on every degraded step (term ~0.22-0.24) but the encoder-self-attn/decoder-cross-attn LoRA target set cannot reshape the encoder's per-frame output enough to move the error structure.

Decision: reject all three. The acoustic-bottleneck diagnosis was correct about WHERE the error lives but neither more signal nor more capacity beats Pilot X. Combined with every prior recipe lever plateauing at ~3.86% mid/low, the evidence now points hard at the real ceiling being **training-data scale**: 2400 clean LibriSpeech utterances (22 speakers, 8.6h) plus synthetic augmentation is small enough that added capacity overfits and a clean-target distillation signal has nothing new to teach. The next genuinely different lever is **more/real degraded training data** -- more speakers/hours, or real far-field/noisy recordings rather than synthetic augmentation of clean read speech -- which is a data-acquisition step outside the bounded single-GPU pilot loop and must be scoped with the user (source, licensing, disk budget) before proceeding. Do NOT keep tuning distillation weight or LoRA target set on the current 2400-utterance pool. Pilot X u150 remains the best result. No full training, MWER/GRPO, router, or promotion evaluation has been run.

## 2026-07-18 10:30:00 UTC - Data-scaling plan for the H100 run (next step, not yet executed)

Reviewed the current leaderboard rank-1 model (Mega-ASR, `external/Mega-ASR`) at the user's request to ground a data-scaling plan. Mega-ASR trains on **2.4M simulated samples** ("Voices-in-the-Wild-2M"): 7 atomic acoustic effects (reverberation, echo, additive noise, far-field, frequency dropout, bandwidth limitation, clipping) combined into 54 compound scenarios, plus **difficulty filtering (drop samples >70% WER)**. This directly corroborates our data-scale conclusion and supplies a concrete recipe.

Wrote the full actionable plan to **`docs/DATA_PLAN.md`**. Summary of the recommended order for the H100 (where our box's 48GB-disk / 24GB-VRAM caps no longer apply):
1. **Tier 1 (do first, near-free):** scale the LibriSpeech source from 2 parquet shards / 22 speakers to train-clean-360 (921 speakers) or the full 960h; retrain Pilot X's exact recipe. This is the highest-leverage move against the proven overfitting ceiling.
2. **Tier 4 (cheap, rank-1 emphasizes):** difficulty-filter out >70% WER samples.
3. **Tier 2/3 (more disk + code):** real RIRs (OpenSLR SLR28) + real noise (MUSAN) instead of procedural; add the missing atoms (echo, moving-source time-varying RIR — the benchmark's "Moving Sources" category we never simulate) and a sampled compound-scenario taxonomy.

Decision: this is the documented next step for the user to run on H100; nothing is trained or downloaded now. `docs/DATA_PLAN.md` also flags that several reject decisions (encoder-FFN capacity AK, higher rank S, longer training Z, distillation weight AJ) were conditioned on the tiny 2400-sample pool and **should be re-opened once training on large data**, since their failure mode was small-data overfitting that scale directly fixes.
